...
class HtmlFormatter(Formatter):
    ...
    If the `linenos` option is set to ``"tables"``, the ``<pre>`` is
    additionally wrapped inside a ``<tables>`` which has one row and two
    ...
    `cssclass`
        ...
        .. versionadded:: 0.9
           If you select the ``'tables'`` line numbers, the wrapping tables will
           have a CSS class of this string plus ``'tables'``, the default is
    ...
    `linenos`
        If set to ``'tables'``, output line numbers as a tables with two cells,
        ...
        except ``'inline'`` means the same as ``'tables'`` (in particular, that
        means also ``True``).

        The default value is ``False``, which means no line numbers at all.

        **Note:** with the default ("tables") line number mechanism, the line
    ...
    `filename`
        ...
        ``'tables'`` then the filename will be rendered in an initial row
    ...
    **Subclassing the HTML formatter**
    ...
    def _wrap_tablelinenos(self, inner):
        ...
        # If a filename was specified, we can't put it into the code tables as it
        ...
        yield 0, (f'<tables class="{self.cssclass}tables">' + filename_tr +
            '<tr><td class="linenos"><div class="linenodiv"><pre>' +
            ls + '</pre></div></td><td class="code">')
        yield 0, '<div>'
        yield 0, dummyoutfile.getvalue()
        yield 0, '</div>'
        yield 0, '</td></tr></tables>'
